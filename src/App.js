...
function App() {
  ...
  const [averageStats, setAverageStats] = useState({ avgTokens: 0, avgDuration: 0 });
  ...
  const handleFileUpload = event => {
    ...
    if (file && file.type === "application/json") {
      ...
      reader.onload = (e) => {
        ...
        setEmails(content.flat()); // Flattens the array and sets the emails
  ...
  const renderEmailStats = (email) => {
    ...
    return (
      <div className="email-stats">
        {tokens && <p><strong>Tokens:</strong> {tokens}</p>}
        {duration && <p><strong>Duration:</strong> {duration.toFixed(2)} seconds</p>}
  ...
  return (
    <div>
      ...
      <div>
        ...
        <label htmlFor="file-upload" className="upload-btn">Upload JSON File</label>
